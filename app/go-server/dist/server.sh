...
CMD="$JAVA_HOME/bin/java -server $YOURKIT -Xms$SERVER_MEM -Xmx$SERVER_MAX_MEM -XX:PermSize=$SERVER_MIN_PERM_GEN -XX:MaxPermSize=$SERVER_MAX_PERM_GEN $JVM_DEBUG $GO_SERVER_SYSTEM_PROPERTIES -Duser.language=en -Dorg.mortbay.jetty.Request.maxFormContentSize=30000000 -Duser.country=US -Dcruise.config.dir=$GO_CONFIG_DIR -Dcruise.config.file=$GO_CONFIG_DIR/cruise-config.xml -Dcruise.server.port=$GO_SERVER_PORT -Dcruise.server.ssl.port=$GO_SERVER_SSL_PORT -jar $SERVER_DIR/go.jar"

echo "Starting with command: $CMD" >>$LOG_FILE
echo "Starting in directory: $GO_WORK_DIR" >>$LOG_FILE
cd "$SERVER_WORK_DIR"
